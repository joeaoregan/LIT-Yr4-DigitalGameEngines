...
//	public Text actionText;															// Display a message for the player
	...
	IEnumerator ObjectiveComplete(){
		//if (!gc.GetComponent<ObjectiveComplete>().get()) {
		...
		if (!gc.GetComponent<ManageObjectives>().GetObjective3Complete()) {
			gc.GetComponent<ManageObjectives> ().SetObjective3Complete (true);				// Mark objecive as completed (stops message showing repeatedly, and incrementing objectives)
